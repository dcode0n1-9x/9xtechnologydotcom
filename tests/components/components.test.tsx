...
describe("CredentialField", () => {
  ...
  it("shows the fallback and hides copy/toggle when the password is missing", () => {
    ...
    expect(screen.getByText("Request from CTO")).toBeInTheDocument();
...
describe("DemoSandbox", () => {
  ...
  it("renders 'Request from CTO' without a password copy action", async () => {
    await renderSandbox("");
    expect(screen.getByText("Request from CTO")).toBeInTheDocument();
...
describe("QrTile", () => {
  it("renders an accessible SVG QR for a valid destination", () => {
    render(<QrTile url="https://example.com/app" label="QR code for the app" />);
    const qr = screen.getByRole("img", { name: "QR code for the app" });
    expect(qr.tagName.toLowerCase()).toBe("svg");
    expect(qr).toHaveAttribute("data-qr-value", "https://example.com/app");
  });
...
describe("SaveContactButton", () => {
  it("downloads the vCard with the expected filename and MIME", () => {
    render(<SaveContactButton />);
    const link = screen.getByRole("link", { name: card.contact.cta });
    expect(link).toHaveAttribute("href", "/hemant.vcf");
    expect(link).toHaveAttribute("download", "Hemant-9xTechnology.vcf");
